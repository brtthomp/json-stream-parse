...
class Parser extends Stream.Writable {
    ...
    _algorithm1a (done) {
        ...
        const indexA = this._buffer.indexOf(`${String.fromCharCode(endChar)}[`, this._searchIndex);
        const indexB = this._buffer.indexOf(`${String.fromCharCode(endChar)}{`, this._searchIndex);
        let index = null;
        if (indexA === -1) {
            index = indexB;
        } else if (indexB === -1) {
